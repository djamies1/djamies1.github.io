#!/usr/bin/env python3
"""daily_upload.py — Pick emoji quiz questions, render, upload to YouTube."""
...
DEFAULT_LIMIT  = 3
YT_SUFFIX = " | Emoji Quiz"
YT_TAGS   = ["emoji quiz","guess the emoji","emoji riddle","shorts","brain teaser",
              "emoji challenge","can you guess","emoji puzzle","fun quiz","emoji game"]
...
def find_video(qid):
    for p in Path(VIDEO_OUTPUT_FOLDER).glob(f"eq_*_{qid}_*.mp4"): return p
    return None
...
def make_path(q, all_q):
    ...
    return out/f"eq_{idx:04d}_{q.get('id','x')}_{safe}.mp4"

def build_title(q):
    hook = q.get("hook","Guess the emoji!")
    suf  = YT_SUFFIX
    return (hook[:100-len(suf)-1]+"…" if len(hook)>100-len(suf) else hook)+suf

def build_desc(q):
    cat = q.get("category","").title()
    return (f"🤔 What {cat or 'thing'} is this?\n\n{q['emojis']}\n\n"
            f"──────────────────────────────\n✅ ANSWER: {q['answer']}\n"
            f"──────────────────────────────\n\nComment your guess before the reveal! 👇\n"
            f"Share to test your friends!\n\nSubscribe → Emoji Quiz\n\n"
            f"#emojiquiz #guesstheemoji #emojiriddle #shorts #brainteaser #emojichallenge")
...
def upload(yt, path, q, privacy, publish_at=None):
    ...
    body = {"snippet":{"title":build_title(q),"description":build_desc(q),"tags":YT_TAGS,"categoryId":"24"},"status":sb}
...
def main():
    ...
    args = ap.parse_args()
    qs = load_q(); u = load_u()
    if not qs: sys.exit("No questions.")
    ...
    for i,q in enumerate(to_do,1): print(f"  {i}. {q['emojis']} → {q['answer']}")
    print()
    if args.dry_run:
    ...
    for i,q in enumerate(to_do):
        print(f"\n{'='*55}\n[{i+1}/{len(to_do)}] {q['emojis']} → {q['answer']}\n{'='*55}")
        ...
        u[q["id"]]={"youtube_id":vid,"youtube_url":f"https://www.youtube.com/watch?v={vid}",
                     "emojis":q["emojis"],"answer":q["answer"],
    ...
    print(f"\nDone. {len(u)} emoji quiz video(s) total.")
