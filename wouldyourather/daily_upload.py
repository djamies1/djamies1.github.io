#!/usr/bin/env python3
"""
daily_upload.py — Pick unuploaded WYR questions, render videos, upload to YouTube.
...
YT_TITLE_SUFFIX = " | Would You Rather"
YT_TAGS = [
    "would you rather", "wyr", "this or that", "dilemma", "shorts",
    "would you rather challenge", "impossible choice", "pick one",
    "brain teaser", "hypothetical", "debate",
]
...
def find_existing_video(qid: str) -> Path | None:
    ...
    for path in out_dir.glob(f"wyr_*_{qid}_*.mp4"):
        return path
    return None
...
def make_output_path(question: dict, all_questions: list[dict]) -> Path:
    ...
    return out_dir / f"wyr_{index:04d}_{qid}_{safe}.mp4"


# ── YouTube metadata ───────────────────────────────────────────────────────────

def build_title(question: dict) -> str:
    hook   = question.get("hook", "Would you rather...")
...
def build_description(question: dict) -> str:
    ...
    if note:
        desc += f"💬 {note}\n\n"
    desc += (
        f"Comment A or B below — no wrong answers! 👇\n\n"
        f"Subscribe for a daily dilemma → Would You Rather\n\n"
        f"#{cat.replace(' ', '')} #wouldyourather #wyr #thisorthat #shorts #debate #hypothetical"
    )
    return desc
...
def upload_video(youtube, video_path: str, question: dict, privacy: str,
    ...
    media = MediaFileUpload(
        video_path, mimetype="video/mp4", resumable=True, chunksize=256 * 1024,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Pick WYR questions, render, and upload to YouTube.",
    ...
    print(f"{len(candidates)} unuploaded questions available. Processing top {len(to_process)}:\n")
    for i, q in enumerate(to_process, 1):
        ...
        print(f"  {i}. [{q.get('id','')}] A: {q['option_a'][:40]}  ({status})")
    print()
    ...
    for i, question in enumerate(to_process):
        print(f"\n{'='*60}")
        print(f"[{i+1}/{len(to_process)}] A: {question['option_a'][:50]}")
        print(f"{'='*60}")
    ...
    print(f"\nDone. {len(uploaded)} WYR video(s) uploaded to date.")
