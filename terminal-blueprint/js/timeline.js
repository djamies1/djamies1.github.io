...
const SPOT_GROUPS = ['#asm-panel', '#asm-mount', '#roofline', '#asm-indoor', '#asm-net'];
...
function build(isMobile, onProgress) {
  ...
  if (!isMobile) {
    ...
    spot(tl, 14.4, { '#asm-panel': 1 });                        // the array carries scenes 1–3
    spot(tl, 49.2, { '#asm-indoor': 1 });                       // into the home
    spot(tl, 60.2, {});                                         // DETAIL B carries the models scene
    spot(tl, 71.2, { '#asm-net': 1, '#asm-panel': SOFT });      // end-to-end strip
    spot(tl, 82.4, Object.fromEntries(SPOT_GROUPS.map((s) => [s, 1]))); // explode: all lit
  }
