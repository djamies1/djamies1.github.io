#!/usr/bin/env python3
"""daily_upload.py — Pick true/false statements, render, upload to YouTube."""
...
DEFAULT_LIMIT   = 3
YT_SUFFIX = " | True or False?"
YT_TAGS   = ["true or false","trivia","myth busted","shorts","brain teaser",
              "quiz","fact check","did you know","educational","can you guess"]

def load_q(): return json.load(open(STATEMENTS_FILE,encoding="utf-8")) if Path(STATEMENTS_FILE).exists() else []
...
def find_video(qid):
    for p in Path(VIDEO_OUTPUT_FOLDER).glob(f"tof_*_{qid}*.mp4"): return p
    return None
...
def make_path(q, all_q):
    ...
    safe = re.sub(r"[^a-zA-Z0-9 _-]","",q.get("id",f"tof_{idx}")).strip()[:35]
    return out/f"tof_{idx:04d}_{safe}.mp4"

def build_title(q):
    hook = q.get("hook","True or False?")
    suf  = YT_SUFFIX
...
def build_desc(q):
    ...
    return (f"TRUE or FALSE? 🤔\n\n\"{q['statement']}\"\n\n"
            f"──────────────────────────────\n✅ ANSWER: {verdict}\n"
            f"──────────────────────────────\n\n{q.get('explanation','')}\n\n"
            f"Comment your answer before the reveal! 👇\n"
            f"Subscribe for daily true/false challenges!\n\n"
            f"Category: {cat}\n\n"
            f"#trueorfalse #trivia #mythbusted #shorts #brainteaser #factcheck #quiz")
...
def main():
    ...
    args = ap.parse_args()
    qs = load_q(); u = load_u()
    if not qs: sys.exit("No statements.")
    ...
    for i,q in enumerate(to_do,1): print(f"  {i}. [{q.get('verdict','?')}] {q['statement'][:65]}")
    print()
    if args.dry_run:
    ...
    for i,q in enumerate(to_do):
        print(f"\n{'='*55}\n[{i+1}/{len(to_do)}] [{q.get('verdict','?')}] {q['statement'][:45]}\n{'='*55}")
        ...
        u[q["id"]]={"youtube_id":vid,"youtube_url":f"https://www.youtube.com/watch?v={vid}",
                    "statement":q["statement"],"verdict":q.get("verdict","?"),
    ...
    print(f"\nDone. {len(u)} true/false video(s) total.")
