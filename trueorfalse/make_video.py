#!/usr/bin/env python3
"""
make_video.py — Render a 20-second True or False Short.

Phase 1 (0–13s): Statement + "TRUE or FALSE?" prompt
Phase 2 (13–20s): Verdict fades in — GREEN for TRUE, RED for FALSE
                  + explanation text + "Did you get it? 👇"
...
PROMPT_Q_COLOR  = (180, 180, 255)  # soft purple-white
TRUE_COLOR      = (60, 220, 100)   # green
FALSE_COLOR     = (255, 80, 80)    # red
NUANCE_COLOR    = (255, 185, 60)   # amber (for "TRUE — mostly" etc.)
EXPLAIN_COLOR   = (200, 210, 200)  # light grey
PROMPT_COLOR    = (255, 255, 255)
SHADOW_COLOR    = (0, 0, 0)

HOOK_FONT_SIZE      = 58
STATEMENT_FONT_SIZE = 62
PROMPT_Q_FONT_SIZE  = 52
VERDICT_FONT_SIZE   = 90
EXPLAIN_FONT_SIZE   = 36
PROMPT_FONT_SIZE    = 44
LINE_SPACING        = 1.3
OVERLAY_OPACITY     = 175
...
MUSIC_FOLDER = str(Path(__file__).resolve().parent / "tof_music")
MUSIC_VOLUME = 0.45
...
def _verdict_color(verdict: str):
    """Return (color, display_text) based on verdict string."""
    v = verdict.upper()
    if v.startswith("TRUE"):
        return TRUE_COLOR, verdict.upper()
    if v.startswith("FALSE"):
        return FALSE_COLOR, verdict.upper()
    # Nuanced verdicts like "TRUE — mostly"
    return NUANCE_COLOR, verdict.upper()
...
def _content_to_image_prompt(item):
    keywords = " ".join(item.get("statement", "").split()[:6])
    cat = item.get("category", "facts")
    return (f"bright vivid colorful bold background, true or false facts, "
            f"{cat}, {keywords}, dramatic, vibrant, engaging, digital art")
...
def _build_narration_text(item) -> str:
    return f"True or False? {item['statement']}"
...
def render_phase1(q):
    """Statement + TRUE or FALSE? prompt."""
    img  = Image.new("RGBA",(WIDTH,HEIGHT),(0,0,0,0))
    draw = ImageDraw.Draw(img)

    # Hook
    hf  = _load_font(HOOK_FONT_SIZE, display=True)
    hl  = _wrap(q.get("hook","True or False?"), hf, DRAW_WIDTH)
    ...
    # Statement
    sf  = _load_font(STATEMENT_FONT_SIZE, display=True)
    sl  = _wrap(f'"{q["statement"]}"', sf, DRAW_WIDTH)
    slh = int(STATEMENT_FONT_SIZE*LINE_SPACING)
    sw, sh = _block_size(sl, sf, slh)
    _draw_panel(draw, CENTER_X, int(HEIGHT*STATEMENT_Y_FRAC), sw, sh)
    _draw_block(draw, sl, sf, CENTER_X, int(HEIGHT*STATEMENT_Y_FRAC), slh, STATEMENT_COLOR)

    # TRUE or FALSE? prompt
    pf  = _load_font(PROMPT_Q_FONT_SIZE, display=True)
    pl  = _wrap("TRUE  or  FALSE?", pf, DRAW_WIDTH)
    plh = int(PROMPT_Q_FONT_SIZE*LINE_SPACING)
    _draw_block(draw, pl, pf, CENTER_X, int(HEIGHT*PROMPT_Q_Y_FRAC), plh, PROMPT_Q_COLOR, alpha=200)

    return np.array(img)


def render_verdict(q):
    """Verdict in green/red + explanation."""
    img  = Image.new("RGBA",(WIDTH,HEIGHT),(0,0,0,0))
    draw = ImageDraw.Draw(img)

    verdict = q.get("verdict","?")
    color, display = _verdict_color(verdict)

    # Verdict (large)
    vf  = _load_font(VERDICT_FONT_SIZE, display=True)
    vl  = _wrap(display, vf, DRAW_WIDTH)
    vlh = int(VERDICT_FONT_SIZE*LINE_SPACING)
    vw, vh = _block_size(vl, vf, vlh)
    _draw_panel(draw, CENTER_X, int(HEIGHT*VERDICT_Y_FRAC), vw, vh, py=30)
    y = int(HEIGHT*VERDICT_Y_FRAC) - len(vl)*vlh//2
    for line in vl:
        w = int(draw.textlength(line, font=vf))
        x = CENTER_X - w//2
        draw.text((x, y), line, font=vf, fill=(*color, 255),
    ...
    if exp:
        ef  = _load_font(EXPLAIN_FONT_SIZE)
        el  = _wrap(exp, ef, DRAW_WIDTH)
        elh = int(EXPLAIN_FONT_SIZE*LINE_SPACING)
        ew, eh = _block_size(el, ef, elh)
        _draw_panel(draw, CENTER_X, int(HEIGHT*EXPLAIN_Y_FRAC), ew, eh, alpha=120)
        _draw_block(draw, el, ef, CENTER_X, int(HEIGHT*EXPLAIN_Y_FRAC), elh, EXPLAIN_COLOR, alpha=210)
...
def render_prompt():
    ...
    l = _wrap("Did you get it?", f, DRAW_WIDTH)
...
def create_video(question, output_path, music_path=MUSIC_FOLDER, music_volume=MUSIC_VOLUME, narration=True):
    print(f'\nRendering: {question["statement"][:60]}...')
    ...
    vr = render_verdict(question); vrf,vra = vr[:,:,:3].astype(np.float32), vr[:,:,3:4].astype(np.float32)/255
    ...
    def make_frame(t):
        ...
        if t >= REVEAL_T:
            ...
            ev = vra*af; frame = frame*(1-ev)+vrf*ev
...
def load_questions():
    p = Path(STATEMENTS_FILE)
    if not p.exists(): sys.exit("statements.json not found.")
    with open(p, encoding="utf-8") as f: return json.load(f)


def _make_path(index, q):
    out = Path(VIDEO_OUTPUT_FOLDER); out.mkdir(parents=True, exist_ok=True)
    safe = re.sub(r"[^a-zA-Z0-9 _-]","",q.get("id",f"tof_{index}")).strip()[:35]
    return str(out/f"tof_{index:04d}_{safe}.mp4")
...
def main():
    ...
    if args.list:
        for i,q in enumerate(qs):
            print(f"{i:<5} [{q.get('verdict','?')}] {q['statement'][:65]}")
        return
