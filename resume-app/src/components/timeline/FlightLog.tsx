...
/* Direction-aware crossfade: stepping forward slides up, back slides down. */
const cardVariants = {
...
/**
 * Career timeline as a hover-scroll carousel: the page scrolls past the
 * section normally, but a wheel gesture while hovering the stage steps
 * through roles (falling through at either end). Touch devices swipe
 * the card or tap the rail; no scroll capture there.
 */
export function FlightLog() {
  ...
  const jump = useCallback((i: number) => {
    const next = Math.max(0, Math.min(JOBS.length - 1, i));
  ...
  // Wheel over the stage steps through roles instead of scrolling the
  // page. At either end the event is left alone so the page keeps
  // moving; React's synthetic onWheel is passive, hence the manual
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const atEdge =
        (dir < 0 && idxRef.current === 0) ||
        (dir > 0 && idxRef.current === JOBS.length - 1);
      if (atEdge) return;
      e.preventDefault();
      ...
      if (Math.abs(accRef.current) >= 50) {
        accRef.current = 0;
        lockRef.current = now + 450;
        jump(idxRef.current + dir);
      }
    };
  ...
  const railProgress = JOBS.length > 1 ? idx / (JOBS.length - 1) : 1;
  const springTransition = reducedMotion
    ? { duration: 0 }
    : { type: "spring" as const, stiffness: 140, damping: 26 };

  return (
    <section className="mx-auto max-w-6xl px-6 py-24" id="flight-log">
      <SectionHeading
        eyebrow="02 · Experience"
        lede="Newest first. Scroll over the card to step through the roles, or jump from the rail."
      ...
      <div ref={stageRef}>
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        borderColor: active ? a : "rgba(255,255,255,0.3)",
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[250px_minmax(0,1fr)]">
          {/* vertical rail with role titles (desktop) */}
          <div className="relative hidden lg:block">
            ...
            <motion.div
              ...
              transition={springTransition}
            />
            <ol className="flex h-[420px] flex-col justify-between">
              {JOBS.map((j, i) => {
                const a = ACCENT_HEX[j.accent] ?? "#c9a84c";
                const active = i === idx;
                return (
                  <li key={j.id}>
                    ...
                    >
                      <span
                        className="mt-[3px] h-[11px] w-[11px] shrink-0 rounded-full border transition-all duration-300"
                        style={{
                          background: active ? a : "#141414",
                          borderColor: active ? a : "rgba(255,255,255,0.3)",
                          boxShadow: active ? `0 0 10px ${a}66` : "none",
                      ...
                      <span className="min-w-0">
                        <span
                          className={cn(
                            "block font-mono text-[0.68rem] transition-colors duration-300",
                            active
                              ? "text-gold-light"
                        ...
                        <span
                          className={cn(
                            "block text-xs leading-snug transition-colors duration-300",
                            active
                              ? "text-cream"
                              : "text-muted-foreground/80 group-hover:text-cream/70"
                          )}
                        >
                          {j.role}
                        </span>
          ...
          <div className="relative">
            ...
            <div className="grid min-h-[420px] sm:min-h-[380px] md:min-h-[340px]">
